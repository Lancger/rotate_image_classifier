...
if __name__ == '__main__':
    ...
    # 随机生成旋转图片
    for _ in range(min(10, total_samples)):  # 确保不会超过总样本数
        index = random.randint(0, total_samples - 1)  # 修改随机范围
        img, target = train_dataset[index]
        img = np.reshape(img, (244, 244, 3)).astype(np.float32)
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        cv2.imwrite(f'./img_examples/angle_{target}.png', img)
        print(f"已保存图片 angle_{target}.png")